...
void Fichero::EscribirFicheroMod(std::vector<std::string> alfabeto, std::vector<std::string> cadena, std::vector<std::string> resultado) {
  for(int i = 0; i < alfabeto.size(); i++) {
    fichero_escritura << alfabeto[i] << " -- " << cadena[i] << " -- " << resultado[i] << "\n";
  }
}